...
| 1.10    | 2026-09-21 | Story S1-07 (issue #63) delivered: every `memo search` invocation now carries a fresh, random `query_id` (UUID v4) at the envelope level — inert in Phase 1 (nothing persisted, nothing read back), establishing the contract Phase 3 fills in. `--explain` projects the full factor bag (`similarity`, `recency_score`, `source_score`, `tag_boost`, `lexical_boost`, plus the explicit Phase-2/3-neutral `retention: 1.0`, `use_ratio: 0`, `link_factor: 1.0`) onto every `--json` result and appends an aligned factor table plus `query_id` footer line in human mode; it changes no ordering, no scores, and issues no extra Qdrant/embeddings call. This entry backfills a changelog gap: S1-07 shipped without its own row before this exit-gate pass.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | developer        |
| 1.11    | 2026-09-21 | Story S1-08 (issue #64) — Phase 1 exit gate: `pnpm run eval:relevance` run live against `memo_eval` (44 entries / 28 queries) with every Phase 1 feature enabled (lexical on, default weights/thresholds) measured **96.4%** overall (concept 100%, identifier 100%, cross-repo 83.3%, recency 100%) — meeting AC-1.5 (≥ 80%, and ≥ the recorded 96.4% baseline, exactly at the floor) with no default tuning required; `candidates.json`/`baseline.json` are unchanged from S1-06's recording. `cross-repo` remains the lowest-scoring category (83.3%, one of six `cross-repo` queries misses its top-3 target) across every Phase 1 story's measurement to date — recorded here as input to §18 Q3 (recency-vs-retention tuning is deferred to Phase 3, and this category's persistent gap is a candidate signal for that tuning pass, not a Phase 1 defect). Doc sweep across `README.md`, `docs/data-model.md`, `docs/system-overview.md`, `docs/technical-guidelines.md` closed drift: the local-config table in `docs/data-model.md` was missing six shipped `ranking.*` fields (`tag_boost_factor`, `confidence_thresholds`, `staleness_threshold_days`, `staleness_tag_overlap_threshold`, `lexical`, `lexical_boost_factor`), the architecture trees in `docs/technical-guidelines.md` were missing four `src/lib/` modules (`facets.ts`, `ranking.ts`, `staleness.ts`, `lexical.ts`), and `docs/system-overview.md`'s search-flow steps stopped short of `query_id`/`--explain` (issue #63). `/TESTING.md` was a full mismatch (described an unrelated project, `@llipe.com/dev-tasks`/Vitest) and is now rewritten to describe this repository's real Jest-based harness, environment/credential requirements, and current coverage numbers. Live `pnpm run test:coverage` measured global `82.79%` statements / `74.69%` branches / `79.66%` functions / `83.59%` lines against `jest.config.ts`'s `80/75/80/80` thresholds — **branches and functions fail the configured gate** (pre-existing debt, not introduced by Phase 1; lowest files are `setup.ts`, `write.ts`, `retry.ts`, `embeddings.ts`, none touched by S1-01–S1-08); `src/lib/` alone measured `89.9%`/`78.98%`/`93.1%`/`92.23%`, clearing the 85% target on statements/functions/lines but not branches. `coverage_gate` is recorded honestly as `FAIL`, not silently passed. `memo search`/`list`/`read`/`write` manually smoke-tested live against `memo_eval` with no regression in default human output. Release `v1.2.0` is deliberately **not** tagged/published by this story — that is routed to the user/planner as a separate, explicit decision after this PR is reviewed. | developer        |
| 1.12    | 2026-09-21 | Phase 2 spec pass (spec v1.2 §18). Open questions resolved by user decision: **Q1** private episodic purge default stays **on** (`purge_after_days: 30`); **Q5** `self` soft cap confirmed at **50**, exposed as config `banks.private.self.soft_cap`, warning-only — a lower cap tends toward more meaningful self-reflection, a higher one risks part of `SELF` being ignored; neither has been tuned against a real long-lived agent yet. Phase 1 closed at v1.2.0 (PR #74). Phase 2 stories (`user-stories-prd-004-phase-2.md` v1.0, S2-01–S2-11) generated; **`memo restore --id` (FR-2.7 second half) moves to Phase 3** — Phase 2 produces no archived entries, so it would be inert; it ships with `decay`/`forget` (FR-3.4/3.5).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | product-engineer |
| 1.13    | 2026-09-22 | Story S2-11 (issue #90) — **Phase 2 exit gate.** All ten Phase 2 stories (S2-01…S2-10) plus one drift fix (S2-06/D-2, PR #99) merged into the integration branch. **AC1**: live `MEMO_COLLECTION=memo_eval pnpm run eval:relevance`, with every Phase 2 feature enabled, measured **96.4%** overall (concept 100%, identifier 100%, cross-repo 83.3%, recency 100%) — exactly matching the recorded Phase 1 floor, no default-weight retuning required. This number was reached only after resolving a process incident, recorded here as a Phase 3+ input: `memo_eval` had accumulated **12 non-fixture points** from five separate stories' AC6 manual-smoke steps (S2-04's `jarvis-eval-s204` bank, dev-tasks ISSUE-89's `developer-memory` bank, S2-08's `smoke-bank`, plus 3 points from this story's own required S2-11 AC6 smoke test — `bad3dabe-...` episodic, `e63ad173-...` self, `f1eab8f2-...` semantic in `kb`) on top of the 44 official fixture points; `scripts/eval-relevance.ts` queries with no payload filter by design, so the 12 leftover points diluted the top-10 dense candidate pool and dropped the first measurement to 82.1% (concept 87.5%, identifier 100%, cross-repo 33.3%, recency 100%) — not a ranking regression. Per this project's blocked-guard rule, no agent attempted a `memo delete` workaround; the user was asked, explicitly enumerated all 12 ids via a read-only Qdrant scroll diffed against the 44 official fixture ids in `tests/fixtures/relevance/entries.json`, and approved deletion. `planner` (not `developer`) ran the 12 `memo delete --id <id> --yes --json` calls, confirmed `memo_eval` back at exactly 44 points/zero non-fixture, and this story re-measured clean: 96.4%, byte-identical to the pre-pollution floor. **AC2**: `tests/relevance/replay.test.ts` (1051/1051 total suite tests) passes unweakened. **AC3**: `pnpm run validate` (typecheck/lint/format:check/test/audit) all pass; `pnpm run test:coverage` global gate **PASSES** at 92.15%/80.35%/89.17%/93.29% (statements/branches/functions/lines) against the 80/75/80/80 threshold, resolving Phase 1's recorded `setup.ts` debt (now 96.12%); `write.ts` remains below an informal per-file target (75.71%/76.11%/52.38%/75.72%) but the repo's `jest.config.ts` enforces only the global threshold, so this does not fail the gate — recorded honestly as a carried-forward limitation, not silently resolved. **AC4**: doc sweep closed drift in `README.md` and `docs/technical-guidelines.md` (stale project-tree tables missing `bank.ts`/`migrate.ts`/`recall.ts`/`timeline.ts` and several `src/lib/` modules; missing `MEMO_COLLECTION`/`MEMO_BANK` env var rows). **AC5**: `docs/adr/ADR-003-one-collection-bank-kind-payload-isolation.md` added, plus one `kb` semantic memo entry citing it. **AC6**: full manual smoke transcript against `memo_eval` — `write` (all three kinds), `search`/`list`/`read`/`timeline`/`recall` (private and `kb`, bank isolation confirmed), `bank list`/`bank show`, `migrate --dry-run` second run `scanned: 0` — no regression in default human output (the smoke test's own 3 leftover points are the ones folded into the AC1 cleanup above). **AC7**: this row, plus PRD §13 Phase 2 boxes AC-2.1–AC-2.9 ticked below, plus a 1.3.0 release-notes draft (`workstream/release-notes-1.3.0-draft.md`) restating that `memo migrate --to-v2` against the production `decisions` collection remains a separate, explicit, human-run step, never automatic. **AC8**: `verifier_audit` is recorded honestly as `not-run(no-delegation)` by this story — per this repository's `developer` agent contract, the PRD-level rollup audit is `planner`'s responsibility, run directly against this story's diff/branch/PR immediately after this closeout, not something `developer` self-certifies. Process learning for Phase 3+: every phase's AC6 "manual smoke against `memo_eval`" step has produced leftover points across two phases now (9 in Phase 2 before this story's own 3 more) with no accompanying cleanup — a candidate for a dedicated smoke-bank convention or a defensive fixture-id filter in `eval-relevance.ts`, tracked as an open decision, not fixed inline here. | developer        |
...
- [x] AC-1.1 Evaluation set exists with ≥ 20 labeled queries; baseline top-3 hit rate is recorded in this changelog.
- [x] AC-1.2 Under default config, a fresh mid-similarity entry outranks a stale high-similarity one (#34 AC2 scenario).
- [x] AC-1.3 A query containing an exact file name returns the entry listing that file in the top 3; with `--lexical off` it may not.
- [x] AC-1.4 JSON output carries `query_id`, `final_score`, `similarity`, `recency_score`, `source_score`, `tag_boost`, `lexical_boost`, `confidence_tier`, and `stale`/`stale_by` when applicable; existing envelope keys are unchanged.
- [x] AC-1.5 Top-3 hit rate is ≥ 80% or ≥ baseline + 15 points, whichever is lower, and never below baseline.

**Phase 2**

- [x] AC-2.1 `memo write --kind self` in bank `kb` exits 1 with `VALIDATION_FAILED`; with `MEMO_BANK=jarvis-memory` it succeeds and the point carries `bank = jarvis-memory`, `kind = self`. — proved by S2-04 (issue #83, PR #95)
- [x] AC-2.2 `memo write` in a private bank with no `--kind` produces `kind = episodic`; in `kb` it produces `kind = semantic`. — proved by S2-04 (issue #83, PR #95)
- [x] AC-2.3 `memo search` with no new flags returns exactly the entries it returned before migration. — proved by S2-05 (issue #84, PR #96)
- [x] AC-2.4 `memo search --bank a-memory` never returns entries of `b-memory` (tested with two banks). — proved by S2-05 (issue #84, PR #96)
- [x] AC-2.5 `memo timeline --bank x --session s-42` returns entries in `seq` then `timestamp_utc` order regardless of similarity. — proved by S2-06 (issue #85, PR #97 + drift fix PR #99)
- [x] AC-2.6 `memo recall --bank x` returns `SELF` first with every valid `self` entry, omits superseded `self` entries, keeps `SELF` intact under `--max-tokens` trimming, deduplicates ids across sections, and returns one `query_id`. — proved by S2-07 (issue #86, PR #100)
- [x] AC-2.7 `memo migrate --to-v2 --dry-run` writes nothing; a real run applies the FR-2.8 rules; a second run reports 0 changes; no entry is archived or deleted. — proved by S2-09 (issue #88, PR #102)
- [x] AC-2.8 `memo write --kind self --supersedes <id>` hides the old entry from `recall` and `memo bank show`; `memo read` still returns it with `valid_to` set. — proved by S2-04 (issue #83, PR #95) and S2-08 (issue #87, PR #101)
- [x] AC-2.9 dev-tasks `developer` prompt uses `memo recall` at session start and writes intent/outcome entries as episodic in the agent's bank with `--session ISSUE-<n>`. — proved by S2-10 (issue #89, PR #103 + dev-tasks PR #240)
...
3. **Recency vs retention overlap.** Both are time-based. Keep both with defaults and tune on the evaluation set, or lower `w_recency` once retention exists? Default: keep both, tune at Phase 3 exit. **Phase 1 exit-gate data point (task 8.0, changelog 1.11):** across every Phase 1 measurement (S1-02 through S1-08), `cross-repo` has consistently been the lowest-scoring category (66.7-83.3%, versus 100% on `concept`/`identifier`/`recency`), never the deciding factor for the 80%/baseline+15 gate but a persistent gap. This is recorded as input evidence for the Phase 3 tuning pass, not resolved here — Phase 1's fixed formula and this small (44-entry) fixture set do not isolate whether the gap is a `w_recency`/retention interaction, a cross-repo `search_scope` weighting effect, or simply fixture-set noise at this sample size.
...
5. **`self` soft cap.** 50 entries per bank with a warning, no hard limit. Confirm. **Resolved (v1.12): confirmed at 50, warning-only, configurable as `banks.private.self.soft_cap`.** Rationale recorded for future tuning: a lower cap pushes the bank owner toward fewer, more meaningful `self` entries; a higher cap risks part of the `SELF` section being ignored at recall time. The right value has not been tested against a real long-lived agent — treat 50 as a starting point and revisit once Phase 2 has produced real `SELF` sections.
