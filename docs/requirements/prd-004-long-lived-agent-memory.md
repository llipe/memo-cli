...
| 1.7     | 2026-09-21 | Story S1-02 (issue #34) delivered: composite ranking (`final_score = 0.6*similarity + 0.3*recency + 0.1*source`, default weights) lands in `memo search`, superseding raw-similarity ordering. Re-recorded the relevance baseline against the same live Qdrant Cloud fixtures now that the harness itself ranks candidates through the composite score (per §8.2 R6): overall top-3 hit rate moved from **92.9%** (pre-#34, raw similarity) to **85.7%** (post-#34, default weights) — concept 100%→75.0%, identifier 100%→87.5%, cross-repo 66.7%→83.3%, recency 100%→100%. The drop is consistent with the refinement's already-documented R1 risk (recency weighting can bury a genuinely correct older decision) rather than an implementation defect; weights are configurable and `similarity` remains visible in output. Flagged for `product-engineer`/phase-end re-validation (§8.2 R6) rather than blocking this story, since D1-D9's default weights are binding decisions this story does not have authority to change unilaterally. | developer        |
