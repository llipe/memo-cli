...
```plantuml
@startuml Agent Interaction Flow
!theme plain
...
' ─── Agents ───
rectangle "**product-engineer**\n//Preparation//" as PE #E3F2FD
rectangle "**developer**\n//Execution//" as DEV #E8F5E9
rectangle "**planner**\n//Orchestration//" as PLAN #FFF3E0
rectangle "**ux-engineer**\n//UX Validation//" as UX #F3E5F5
rectangle "**github-ops**\n//GitHub Consistency//" as GH #FFF9C4
rectangle "**technical-writer**\n//Documentation//" as TW #E0F7FA
rectangle "**housekeeping**\n//Lint & Type Fixes//" as HK #F1F8E9

' ─── Skills ───
package "Activity Skills" as AS #F5F5F5 {
  card "activity-init" as si
  card "activity-refine" as sr
  card "activity-generate-spec" as sgs
  card "activity-generate-stories" as sst
  card "activity-publish-github" as spg
}

package "Operational Skills" as OS #F5F5F5 {
  card "git-ops" as sgo
  card "memo-cli-usage" as smc
  card "webapp-mockup" as swm
}

' ─── Instructions ───
package "Instructions (always loaded)" as INSTR #EEEEEE {
  card "plan" as ip
  card "implement" as ii
  card "nextjs-pages-components" as inc
}
...
' Full Feature Flow
PE -down-> DEV : "handoff\ntask list"
PE -down-> PLAN : "handoff\nmulti-story plan"
PLAN -right-> DEV : "orchestrate\nsequential execution"

' UX Validation Loop
PE -right-> UX : "spec for\nmockups"
UX -up-> PE : "gaps &\nrefinements"

' Post-Implementation
DEV -right-> TW : "feature delivered\n→ update docs"
DEV -down-> HK : "lint/type\nremediation"
DEV --> GH : "branch, PR,\nlabels"
PE --> GH : "issues,\nmilestones"
PLAN --> GH : "checklists,\nprogress"
...
legend right
  ...
  | ··▷ | Skill consumption / Instruction application |
endlegend

note bottom of PE
  **Workflow entry point**
  Refine → Spec → Stories → Plan
end note
