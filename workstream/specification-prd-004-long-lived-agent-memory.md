...
- `list [--json]`: `aggregateField(scroll, 'bank')` with v1 points (no `bank`) folded into `kb`; per bank, three `count()` calls (one per kind, default exclusions). JSON `{ banks: [{ bank, counts: { self, episodic, semantic }, total }] }`.
