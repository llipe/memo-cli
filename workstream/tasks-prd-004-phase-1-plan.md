...
- [ ] 2.0 Implement Story S1-02: Composite ranking score — [#34](https://github.com/llipe/memo-cli/issues/34)
  ...
  - [x] 2.20 Verify AC19–AC21 (added by the GitHub issue's refined-scope comment): AC19 (factor bag) and AC20 (purity) PASS. **AC21 FAILS and is left failing, not silently resolved:** `tests/relevance/replay.test.ts` replays `tests/fixtures/relevance/candidates.json` through the composite ranking and measures 85.7%, below the S1-01 floor (92.9%) recorded in `tests/fixtures/relevance/baseline.json`, which was deliberately NOT overwritten — see 2.22 and the PR body for the full analysis and routing to `product-engineer`
  ...
  - [x] 2.22 Run `pnpm run eval:relevance`; state before and after hit rates in the PR body — required updating `scripts/eval-relevance.ts`'s live-run `toQueryResults` and `tests/relevance/replay.test.ts`'s offline replay to rank candidates through `rankResults` before slicing top-3 (they previously used raw Qdrant/similarity order and so were structurally incapable of measuring any ranking-affecting story, defeating PRD §8.2 R6's "re-validated at the end of every phase"). Before (raw similarity, S1-01 recorded floor): **92.9%**. After (composite ranking, default weights, measured via offline replay): **85.7%** — this is BELOW the floor (AC21 fails). Per explicit user/planner decision, `tests/fixtures/relevance/baseline.json` and `candidates.json` were **NOT** overwritten with the 85.7% run — the 92.9% regression floor from S1-01 stays intact and `replay.test.ts`'s `>= baseline` assertions are left genuinely failing (2/347 tests), not weakened or skipped. See PR body for the full before/after breakdown, root-cause analysis, and routing to `product-engineer`
  ...
  - [x] 2.26 Run quality gate: `pnpm run lint && pnpm run format:check && pnpm run typecheck && pnpm test && pnpm audit` — lint/typecheck/audit green; `format:check` clean on every file this story touches (repo-wide fails on pre-existing drift). `pnpm test`: **345/347 pass, 2 fail by design** — the two failures are `tests/relevance/replay.test.ts`'s AC21 gate (see 2.20/2.22), deliberately left failing per explicit user/planner decision rather than papered over by weakening the baseline; every other test is green. Repo-wide `test:coverage` also fails on a pre-existing shortfall that predates this story (see PR body)
