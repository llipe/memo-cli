...
- [ ] 2.0 Implement Story S1-02: Composite ranking score — [#34](https://github.com/llipe/memo-cli/issues/34)
  ...
  - [x] 2.20 Verify AC19–AC21 (not in the refinement's AC1-18 list; treated as this task's own additional verification per its wording): purity of `ranking.ts` (no I/O, no ambient clock reads, `now` injected, confirmed by code review and tests), factor bag shape (`RankingFactors` neutral defaults, tested), replay hit rate — see 2.22, does NOT hold ≥ the pre-#34 baseline; see PR body
  ...
  - [x] 2.22 Run `pnpm run eval:relevance`; state before and after hit rates in the PR body — required updating `scripts/eval-relevance.ts` and `tests/relevance/replay.test.ts` to rank candidates through `rankResults` before slicing top-3 (they previously used raw Qdrant/similarity order and so were structurally incapable of measuring any ranking-affecting story, defeating PRD §8.2 R6). Before (raw similarity, S1-01): 92.9%. After (composite ranking, default weights): 85.7%. See PR body for the full before/after breakdown and root-cause analysis
