...
AC-13's formula `max(limit, min(limit * 3, 50))` is unbounded above, because the outer `max` defeats the cap for any `limit > 50`. Verified: `limit = 1_000_000` → over-fetch `1_000_000`. AC-14 explicitly _requires_ this (`--limit 100` must fetch `100`, not `50`), so the issue body's "capped at 50" intent cannot hold simultaneously. This is a requirement conflict, not an implementation defect. EC-18 stays `undetermined` until `product-engineer` decides whether to add an absolute ceiling or bound `--limit` itself.
